...
// ── v3.41.0: Feature Importance Ranking ────────────────
async function runFeatureRanking() {
    showLoading();
    var results = document.getElementById('feature-ranking-results');
    results.style.display = 'none';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var r = await apiFetch('/api/analysis/feature-ranking', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({source: 'demo', well: well})
        });
        results.style.display = '';
        var html = '<h5>Feature Importance Ranking — Well ' + r.well + '</h5>';
        html += '<div class="row mb-3">';
        html += '<div class="col-md-4"><div class="card"><div class="card-body text-center">';
        html += '<h6>Features Analyzed</h6><h2>' + (r.n_features || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-4"><div class="card"><div class="card-body text-center">';
        html += '<h6>Samples</h6><h2>' + (r.n_samples || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-4"><div class="card"><div class="card-body text-center">';
        html += '<h6>Top Feature</h6><span class="badge bg-primary fs-6">' + (r.top_5 && r.top_5[0] || 'N/A') + '</span>';
        html += '</div></div></div></div>';
        if (r.features && r.features.length) {
            html += '<h6>Top 15 Features</h6><table class="table table-sm table-bordered"><thead><tr><th>#</th><th>Feature</th><th>Consensus</th><th>RF</th><th>Permutation</th><th>Correlation</th><th>Agreement</th></tr></thead><tbody>';
            r.features.slice(0, 15).forEach(function(f, i) {
                var agr = f.agreement === 3 ? '🟢 3/3' : f.agreement === 2 ? '🟡 2/3' : '🔴 ' + f.agreement + '/3';
                html += '<tr><td>' + (i+1) + '</td><td>' + f.feature + '</td><td>' + f.consensus_score + '</td><td>' + f.rf_importance + '</td><td>' + f.permutation_importance + '</td><td>' + f.correlation + '</td><td>' + agr + '</td></tr>';
            });
            html += '</tbody></table>';
        }
        if (r.recommendations && r.recommendations.length) {
        ...
        results.innerHTML = html;
        if (r.plot) {
            var plotEl = document.getElementById('feature-ranking-plot');
            if (plotEl) { plotEl.src = r.plot; plotEl.style.display = ''; }
        }
    } catch (e) {
        results.style.display = '';
...
// ── v3.41.0: Cluster Stability Analysis ────────────────
async function runClusterStability() {
    showLoading();
    var results = document.getElementById('cluster-stability-results');
    results.style.display = 'none';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var r = await apiFetch('/api/analysis/cluster-stability', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({source: 'demo', well: well})
        });
        results.style.display = '';
        var html = '<h5>Cluster Stability — Well ' + r.well + '</h5>';
        html += '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Best k</h6><h2>' + (r.best_k || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Silhouette</h6><h2>' + (r.best_silhouette || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Elbow k</h6><h2>' + (r.elbow_k || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Stability</h6><span class="badge bg-' + (r.stability === 'HIGH' ? 'success' : r.stability === 'MODERATE' ? 'warning' : 'danger') + ' fs-6">' + (r.stability || 'N/A') + '</span>';
        html += '</div></div></div></div>';
        if (r.k_results && r.k_results.length) {
            html += '<h6>Results per k</h6><table class="table table-sm table-bordered"><thead><tr><th>k</th><th>Silhouette</th><th>Inertia</th></tr></thead><tbody>';
            r.k_results.forEach(function(kr) {
                var bold = kr.k === r.best_k ? 'fw-bold' : '';
                html += '<tr class="' + bold + '"><td>' + kr.k + '</td><td>' + kr.silhouette + '</td><td>' + kr.inertia + '</td></tr>';
            });
            html += '</tbody></table>';
        }
        if (r.recommendations && r.recommendations.length) {
        ...
        results.innerHTML = html;
        if (r.plot) {
            var plotEl = document.getElementById('cluster-stability-plot');
            if (plotEl) { plotEl.src = r.plot; plotEl.style.display = ''; }
        }
    } catch (e) {
        results.style.display = '';
...
// ── v3.41.0: Well Similarity Matrix ────────────────────
async function runWellSimilarity() {
    showLoading();
    var results = document.getElementById('well-similarity-results');
    results.style.display = 'none';
    try {
        var r = await apiFetch('/api/analysis/well-similarity', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({source: 'demo'})
        });
        results.style.display = '';
        var html = '<h5>Well Similarity Matrix</h5>';
        html += '<div class="row mb-3">';
        html += '<div class="col-md-6"><div class="card"><div class="card-body text-center">';
        html += '<h6>Wells Compared</h6><h2>' + (r.n_wells || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-6"><div class="card"><div class="card-body text-center">';
        html += '<h6>Can Share Models</h6><h2>' + (r.pairs ? r.pairs.filter(function(p) { return p.can_share_model; }).length : 0) + '/' + (r.pairs ? r.pairs.length : 0) + '</h2>';
        html += '</div></div></div></div>';
        if (r.pairs && r.pairs.length) {
            html += '<h6>Well Pairs</h6><table class="table table-sm table-bordered"><thead><tr><th>Well A</th><th>Well B</th><th>Similarity</th><th>Type Overlap</th><th>Share Model?</th></tr></thead><tbody>';
            r.pairs.forEach(function(p) {
                var badge = p.can_share_model ? 'success' : 'danger';
                html += '<tr><td>' + p.well_a + '</td><td>' + p.well_b + '</td><td>' + p.similarity + '</td><td>' + p.type_overlap + '</td><td><span class="badge bg-' + badge + '">' + (p.can_share_model ? 'YES' : 'NO') + '</span></td></tr>';
            });
            html += '</tbody></table>';
        }
        if (r.recommendations && r.recommendations.length) {
        ...
        results.innerHTML = html;
        if (r.plot) {
            var plotEl = document.getElementById('well-similarity-plot');
            if (plotEl) { plotEl.src = r.plot; plotEl.style.display = ''; }
        }
    } catch (e) {
        results.style.display = '';
...
async function runPredictionTimeline() {
    showLoading();
    var results = document.getElementById('prediction-timeline-results');
    results.style.display = 'none';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var r = await apiFetch('/api/analysis/prediction-timeline', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({source: 'demo', well: well})
        });
        results.style.display = '';
        var html = '<h5>Prediction Timeline — Well ' + r.well + '</h5>';
        html += '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Mean Confidence</h6><h2>' + ((r.mean_confidence || 0) * 100).toFixed(0) + '%</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Accuracy</h6><h2>' + (r.accuracy_pct || 0) + '%</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card border-danger"><div class="card-body text-center">';
        html += '<h6>Low Confidence</h6><h2 class="text-danger">' + (r.n_low_confidence || 0) + '</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card border-success"><div class="card-body text-center">';
        html += '<h6>High Confidence</h6><h2 class="text-success">' + (r.n_high_confidence || 0) + '</h2>';
        html += '</div></div></div></div>';
        if (r.low_conf_depth_range) {
            html += '<div class="alert alert-warning"><strong>Low-confidence zone:</strong> ' + r.low_conf_depth_range.min + 'm - ' + r.low_conf_depth_range.max + 'm depth</div>';
        }
        if (r.recommendations && r.recommendations.length) {
            html += '<h6>Recommendations</h6><ul>';
            r.recommendations.forEach(function(rec) { html += '<li>' + rec + '</li>'; });
        ...
        results.innerHTML = html;
        if (r.plot) {
            var plotEl = document.getElementById('prediction-timeline-plot');
            if (plotEl) { plotEl.src = r.plot; plotEl.style.display = ''; }
        }
    } catch (e) {
        results.style.display = '';
...
async function runAugmentationPreview() {
    showLoading();
    var results = document.getElementById('augmentation-preview-results');
    results.style.display = 'none';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var r = await apiFetch('/api/analysis/augmentation-preview', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({source: 'demo', well: well})
        });
        results.style.display = '';
        var html = '<h5>Augmentation Preview — Well ' + r.well + '</h5>';
        html += '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Baseline Accuracy</h6><h2>' + (r.baseline_accuracy || 0) + '%</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Best Method</h6><span class="badge bg-success fs-6">' + (r.best_method || 'N/A') + '</span>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Best Accuracy</h6><h2>' + (r.best_accuracy || 0) + '%</h2>';
        html += '</div></div></div>';
        html += '<div class="col-md-3"><div class="card"><div class="card-body text-center">';
        html += '<h6>Imbalance Ratio</h6><h2>' + (r.imbalance_ratio || 0) + ':1</h2>';
        html += '</div></div></div></div>';
        if (r.strategies && r.strategies.length) {
            html += '<h6>Strategy Comparison</h6><table class="table table-sm table-bordered"><thead><tr><th>Method</th><th>Synthetic</th><th>Baseline</th><th>Augmented</th><th>Change</th></tr></thead><tbody>';
            r.strategies.forEach(function(s) {
                var cls = s.improvement_pct > 0 ? 'text-success' : 'text-danger';
                html += '<tr><td>' + s.method + '</td><td>' + s.n_synthetic + '</td><td>' + s.baseline_accuracy + '%</td><td>' + s.augmented_accuracy + '%</td><td class="' + cls + '">' + (s.improvement_pct > 0 ? '+' : '') + s.improvement_pct + '%</td></tr>';
            });
            html += '</tbody></table>';
        }
        if (r.recommendations && r.recommendations.length) {
        ...
        results.innerHTML = html;
        if (r.plot) {
            var plotEl = document.getElementById('augmentation-preview-plot');
            if (plotEl) { plotEl.src = r.plot; plotEl.style.display = ''; }
        }
    } catch (e) {
        results.style.display = '';
...
async function runApertureDistribution() {
    ...
    showLoading();
    try {
        var r = await apiFetch('/api/analysis/aperture-distribution-stats', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
async function runStabilityWindow() {
    ...
    showLoading();
    try {
        var r = await apiFetch('/api/analysis/stability-window-map', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
// ═══════════════════════════════════════════════════════════════
async function runApertureDistribution() {
    var results = document.getElementById('apertureDist-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Analyzing aperture distribution...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/aperture-distribution-stats', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('apertureDist-well').value, base_aperture_mm: parseFloat(document.getElementById('apertureDist-base').value) || 0.5 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Mean</strong><br>' + (r.mean_aperture_mm || 0).toFixed(4) + ' mm</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Median</strong><br>' + (r.median_aperture_mm || 0).toFixed(4) + ' mm</div></div>';
        ...
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>P10-P90</strong><br>' + (r.p10_mm || 0).toFixed(3) + ' - ' + (r.p90_mm || 0).toFixed(3) + ' mm</div></div>';
        html += '</div>';
        html += '<div class="row mb-3"><div class="col-md-4"><div class="card p-2 text-center"><strong>Mean k</strong><br>' + (r.mean_permeability_darcy || 0).toFixed(1) + ' darcy</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>Lognormal μ</strong><br>' + (r.lognormal_mu || 0).toFixed(4) + '</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>Lognormal σ</strong><br>' + (r.lognormal_sigma || 0).toFixed(4) + '</div></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3" id="aperture-dist-plot">';
...
// ═══════════════════════════════════════════════════════════════
async function runStabilityWindow() {
    var results = document.getElementById('stabWindow-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Computing stability window...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/stability-window-map', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('stabWindow-well').value, depth: parseFloat(document.getElementById('stabWindow-depth').value) || 3000, ucs_mpa: parseFloat(document.getElementById('stabWindow-ucs').value) || 80, tensile_mpa: parseFloat(document.getElementById('stabWindow-tensile').value) || 5 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Stability</strong><br><span class="badge bg-' + (r.stability_class === 'CRITICAL' ? 'danger' : r.stability_class === 'NARROW' ? 'warning' : r.stability_class === 'MODERATE' ? 'info' : 'success') + '">' + (r.stability_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>% Stable</strong><br>' + (r.pct_stable || 0) + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Min Window</strong><br>' + (r.min_window_SG || 0).toFixed(3) + ' SG</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Max Window</strong><br>' + (r.max_window_SG || 0).toFixed(3) + ' SG</div></div>';
        html += '</div>';
        if (r.best_orientation) html += '<div class="alert alert-success">Best: Az=' + r.best_orientation.azimuth_deg + '°, Inc=' + r.best_orientation.inclination_deg + '° (window ' + r.best_orientation.window_SG + ' SG)</div>';
        if (r.worst_orientation) html += '<div class="alert alert-danger">Worst: Az=' + r.worst_orientation.azimuth_deg + '°, Inc=' + r.worst_orientation.inclination_deg + '° (window ' + r.worst_orientation.window_SG + ' SG)</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3" id="stab-window-plot">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runStressAnisotropy() {
    var results = document.getElementById('stressAniso-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Computing stress anisotropy...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/stress-anisotropy', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('stressAniso-well').value, depth: parseFloat(document.getElementById('stressAniso-depth').value) || 3000 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>H-Anisotropy</strong><br>' + (r.horizontal_anisotropy || 0).toFixed(4) + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Class</strong><br><span class="badge bg-' + (r.anisotropy_class === 'HIGH' ? 'danger' : r.anisotropy_class === 'MODERATE' ? 'warning' : 'success') + '">' + (r.anisotropy_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>R-ratio</strong><br>' + (r.R_ratio || 0).toFixed(4) + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Von Mises</strong><br>' + (r.von_mises_MPa || 0).toFixed(2) + ' MPa</div></div>';
        html += '</div>';
        html += '<div class="row mb-3"><div class="col-md-4"><div class="card p-2 text-center"><strong>Sv</strong><br>' + (r.Sv_MPa || 0) + ' MPa</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>SHmax</strong><br>' + (r.SHmax_MPa || 0) + ' MPa</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>Shmin</strong><br>' + (r.Shmin_MPa || 0) + ' MPa</div></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3" id="stress-aniso-plot">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
// [198] FIT/LOT Prediction (v3.52.0)
// ═══════════════════════════════════════════════════════════════
async function runFITPrediction() {
    var results = document.getElementById('fitPred-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Predicting FIT/LOT...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/fit-prediction', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('fitPred-well').value, depth: parseFloat(document.getElementById('fitPred-depth').value) || 3000, tensile_mpa: parseFloat(document.getElementById('fitPred-tensile').value) || 5 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Pfrac</strong><br>' + (r.Pfrac_MPa || 0) + ' MPa</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Pleak</strong><br>' + (r.Pleak_MPa || 0) + ' MPa</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Safety</strong><br><span class="badge bg-' + (r.safety_class === 'CRITICAL' ? 'danger' : r.safety_class === 'MARGINAL' ? 'warning' : r.safety_class === 'ADEQUATE' ? 'info' : 'success') + '">' + (r.safety_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Margin</strong><br>' + (r.margin_MPa || 0) + ' MPa</div></div>';
        html += '</div>';
        html += '<div class="row mb-3"><div class="col-md-4"><div class="card p-2 text-center"><strong>EMW Frac</strong><br>' + (r.EMW_frac_SG || 0).toFixed(3) + ' SG</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>EMW Leak</strong><br>' + (r.EMW_leak_SG || 0).toFixed(3) + ' SG</div></div>';
        html += '<div class="col-md-4"><div class="card p-2 text-center"><strong>EMW FIT</strong><br>' + (r.EMW_fit_SG || 0).toFixed(3) + ' SG</div></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3" id="fit-pred-plot">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runSusceptibilityMap() {
    var results = document.getElementById('suscMap-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Building susceptibility map...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/susceptibility-map', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('suscMap-well').value, depth: parseFloat(document.getElementById('suscMap-depth').value) || 3000, friction: parseFloat(document.getElementById('suscMap-friction').value) || 0.6 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Risk</strong><br><span class="badge bg-' + (r.risk_class === 'HIGH' ? 'danger' : r.risk_class === 'MODERATE' ? 'warning' : 'success') + '">' + (r.risk_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Grid CS%</strong><br>' + (r.pct_critically_stressed || 0) + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Actual CS%</strong><br>' + (r.pct_actual_cs || 0) + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Max ST</strong><br>' + (r.max_slip_tendency || 0).toFixed(4) + '</div></div>';
        html += '</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3" id="susc-map-plot">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's | Grid: ' + (r.n_grid_cells || 0) + ' cells</small>';
        results.innerHTML = html;
    } catch (e) { results.style.display = ''; results.innerHTML = '<div class="alert alert-danger">Error: ' + e.message + '</div>'; } finally { hideLoading(); }
}
...
// ═══════════════════════════════════════════════════════════════
async function runFracDensityLog() {
    var results = document.getElementById('fracDensLog-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Computing density log...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/fracture-density-log', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('fracDensLog-well').value, bin_size_m: parseFloat(document.getElementById('fracDensLog-bin').value) || 10 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>P10</strong><br>' + (r.P10_per_m || 0).toFixed(4) + '/m</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Mean</strong><br>' + (r.mean_density_per_m || 0).toFixed(4) + '/m</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Max</strong><br>' + (r.max_density_per_m || 0).toFixed(4) + '/m at ' + (r.max_density_depth_m || 0) + 'm</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Distribution</strong><br><span class="badge bg-' + (r.distribution_class === 'HIGHLY_VARIABLE' ? 'danger' : r.distribution_class === 'VARIABLE' ? 'warning' : 'success') + '">' + (r.distribution_class || '-') + '</span></div></div>';
        html += '</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's | ' + (r.n_bins || 0) + ' bins</small>';
        results.innerHTML = html;
...
async function runStressGradient() {
    var results = document.getElementById('stressGrad-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Computing stress gradients...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/stress-gradient', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('stressGrad-well').value, depth_from: parseFloat(document.getElementById('stressGrad-from').value) || 500, depth_to: parseFloat(document.getElementById('stressGrad-to').value) || 5000, n_points: parseInt(document.getElementById('stressGrad-n').value) || 30 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Sv grad</strong><br>' + (r.Sv_gradient_MPa_per_km || 0) + ' MPa/km</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>SHmax grad</strong><br>' + (r.SHmax_gradient_MPa_per_km || 0) + ' MPa/km</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Shmin grad</strong><br>' + (r.Shmin_gradient_MPa_per_km || 0) + ' MPa/km</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Regime</strong><br>' + (r.stress_regime || '-') + '</div></div>';
        html += '</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
...
async function runCoulombFailure() {
    var results = document.getElementById('cff-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Computing CFF...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/coulomb-failure', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('cff-well').value, depth: parseFloat(document.getElementById('cff-depth').value) || 3000, friction: parseFloat(document.getElementById('cff-friction').value) || 0.6, cohesion_mpa: parseFloat(document.getElementById('cff-cohesion').value) || 0 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Failed</strong><br>' + (r.n_failed || 0) + '/' + (r.n_fractures || 0) + ' (' + (r.pct_failed || 0) + '%)</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Class</strong><br><span class="badge bg-' + (r.failure_class === 'EXTENSIVE' ? 'danger' : r.failure_class === 'MODERATE' ? 'warning' : 'success') + '">' + (r.failure_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Mean CFF</strong><br>' + (r.mean_CFF_MPa || 0).toFixed(3) + ' MPa</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Max CFF</strong><br>' + (r.max_CFF_MPa || 0).toFixed(3) + ' MPa</div></div>';
        html += '</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
...
// ═══════════════════════════════════════════════════════════════
async function runFracPorosity() {
    var results = document.getElementById('fracPoro-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Estimating fracture porosity...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/fracture-porosity', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('fracPoro-well').value, aperture_mm: parseFloat(document.getElementById('fracPoro-aperture').value) || 0.5 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Porosity</strong><br>' + (r.fracture_porosity_pct || 0).toFixed(4) + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Class</strong><br><span class="badge bg-' + (r.porosity_class === 'HIGH' ? 'danger' : r.porosity_class === 'MODERATE' ? 'warning' : 'success') + '">' + (r.porosity_class || '-') + '</span></div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Eq. Perm</strong><br>' + (r.equivalent_permeability_darcy || 0).toFixed(4) + ' darcy</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>P10</strong><br>' + (r.P10_per_m || 0).toFixed(4) + '/m</div></div>';
        html += '</div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
...
// ═══════════════════════════════════════════════════════════════
async function runBreakoutAzimuth() {
    var results = document.getElementById('breakoutAz-result');
    results.style.display = ''; results.innerHTML = '<div class="text-center"><div class="spinner-border"></div> Predicting breakout azimuth...</div>';
    showLoading();
    try {
        var resp = await fetch('/api/analysis/breakout-azimuth', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ source: 'demo', well: document.getElementById('breakoutAz-well').value, depth: parseFloat(document.getElementById('breakoutAz-depth').value) || 3000 }) });
        var r = await resp.json();
        if (!resp.ok) { results.innerHTML = '<div class="alert alert-danger">' + (r.error || 'Error') + '</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>SHmax Az</strong><br>' + (r.SHmax_azimuth_deg || 0).toFixed(1) + '°</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Breakout Az</strong><br>' + (r.breakout_azimuth_deg || 0).toFixed(1) + '°</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>DITF Az</strong><br>' + (r.DITF_azimuth_deg || 0).toFixed(1) + '°</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Confidence</strong><br><span class="badge bg-' + (r.confidence === 'LOW' ? 'danger' : r.confidence === 'MODERATE' ? 'warning' : 'success') + '">' + (r.confidence || '-') + '</span></div></div>';
        html += '</div>';
        html += '<div class="row mb-3"><div class="col-md-6"><div class="card p-2 text-center"><strong>Max Hoop</strong><br>' + (r.max_hoop_stress_MPa || 0) + ' MPa at ' + (r.max_hoop_azimuth_deg || 0) + '°</div></div>';
        html += '<div class="col-md-6"><div class="card p-2 text-center"><strong>Min Hoop</strong><br>' + (r.min_hoop_stress_MPa || 0) + ' MPa at ' + (r.min_hoop_azimuth_deg || 0) + '°</div></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
...
// ═══════════════════════════════════════════════════════════════
async function runFracCompliance() {
    var results = document.getElementById('results-fracture-compliance');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('well-select') ? document.getElementById('well-select').value : '3P';
        var bn = parseFloat(document.getElementById('compliance-bn').value) || 1e-10;
        var bt = parseFloat(document.getElementById('compliance-bt').value) || 5e-11;
        var resp = await fetch('/api/analysis/fracture-compliance', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,normal_compliance:bn,tangential_compliance:bt})});
        var r = await resp.json();
        ...
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Anisotropy Ratio</strong><br>' + (r.anisotropy_ratio || 0).toFixed(2) + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Class</strong><br><span class="badge bg-' + (r.anisotropy_class === 'HIGH' ? 'danger' : r.anisotropy_class === 'MODERATE' ? 'warning' : 'success') + '">' + (r.anisotropy_class||'-') + '</span></div></div>';
        html += '</div>';
        if (r.tensor_components) {
            html += '<h6>Tensor Components</h6><table class="table table-sm"><tr><th>S11</th><th>S22</th><th>S33</th><th>S12</th><th>S13</th><th>S23</th></tr>';
            var tc = r.tensor_components;
            html += '<tr><td>'+tc.S11+'</td><td>'+tc.S22+'</td><td>'+tc.S33+'</td><td>'+tc.S12+'</td><td>'+tc.S13+'</td><td>'+tc.S23+'</td></tr></table>';
        }
        if (r.principal_compliances) {
            html += '<h6>Principal Compliances</h6><table class="table table-sm"><tr><th>Axis</th><th>Compliance</th><th>Direction</th></tr>';
            r.principal_compliances.forEach(function(p){ html += '<tr><td>'+p.axis+'</td><td>'+p.compliance.toExponential(3)+'</td><td>['+p.direction.join(', ')+']</td></tr>'; });
            html += '</table>';
        }
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
...
async function runPpDepletion() {
    var results = document.getElementById('results-pp-depletion');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('well-select') ? document.getElementById('well-select').value : '3P';
        var depth = parseFloat(document.getElementById('depletion-depth').value) || 3000;
        var depl = parseFloat(document.getElementById('depletion-mpa').value) || 10;
        var friction = parseFloat(document.getElementById('depletion-friction').value) || 0.6;
        var resp = await fetch('/api/analysis/pp-depletion', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth:depth,depletion_mpa:depl,friction:friction})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Initial CS%</strong><br>' + r.initial_cs_pct + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Final CS%</strong><br>' + r.final_cs_pct + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Change</strong><br>' + (r.cs_change_pct > 0 ? '+' : '') + r.cs_change_pct + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Impact</strong><br><span class="badge bg-' + (r.impact_class === 'SEVERE' ? 'danger' : r.impact_class === 'SIGNIFICANT' ? 'warning' : r.impact_class === 'BENEFICIAL' ? 'success' : 'info') + '">' + (r.impact_class||'-') + '</span></div></div>';
        html += '</div>';
        html += '<div class="row mb-2"><div class="col-md-4"><small>Stress Path A=' + (r.stress_path_coefficient||0).toFixed(4) + '</small></div>';
        html += '<div class="col-md-4"><small>Pp: ' + r.Pp_initial_MPa + ' → ' + r.Pp_final_MPa + ' MPa</small></div>';
        html += '<div class="col-md-4"><small>Sv=' + r.Sv_MPa + ' MPa</small></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
...
// ═══════════════════════════════════════════════════════════════
async function runReactivationPressure() {
    var results = document.getElementById('results-reactivation-pressure');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('well-select') ? document.getElementById('well-select').value : '3P';
        var depth = parseFloat(document.getElementById('reactivation-depth').value) || 3000;
        var friction = parseFloat(document.getElementById('reactivation-friction').value) || 0.6;
        var resp = await fetch('/api/analysis/reactivation-pressure', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth:depth,friction:friction})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Fractures</strong><br>' + r.n_fractures + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Currently Critical</strong><br>' + r.n_currently_critical + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Min Margin</strong><br>' + (r.min_Pp_margin_MPa||0).toFixed(2) + ' MPa</div></div>';
        ...
        html += '</div>';
        html += '<div class="row mb-2"><div class="col-md-4"><small>Min Pp Critical: ' + (r.min_Pp_critical_MPa||0).toFixed(1) + ' MPa</small></div>';
        html += '<div class="col-md-4"><small>Mean Margin: ' + (r.mean_Pp_margin_MPa||0).toFixed(2) + ' MPa</small></div>';
        html += '<div class="col-md-4"><small>Current Pp=' + r.Pp_MPa + ' MPa</small></div></div>';
        if (r.top_vulnerable && r.top_vulnerable.length > 0) {
            html += '<h6>Top Vulnerable Fractures</h6><table class="table table-sm"><tr><th>Az</th><th>Dip</th><th>σn</th><th>τ</th><th>Pp Crit</th><th>Margin</th><th>Critical</th></tr>';
            r.top_vulnerable.slice(0,5).forEach(function(f){ html += '<tr><td>'+f.azimuth_deg+'°</td><td>'+f.dip_deg+'°</td><td>'+f.sigma_n_MPa+'</td><td>'+f.tau_MPa+'</td><td>'+f.Pp_critical_MPa+'</td><td>'+f.Pp_margin_MPa+'</td><td>'+(f.currently_critical?'YES':'no')+'</td></tr>'; });
            html += '</table>';
        }
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
...
// ═══════════════════════════════════════════════════════════════
async function runDeviationSurvey() {
    var results = document.getElementById('results-deviation-survey');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('well-select') ? document.getElementById('well-select').value : '3P';
        var depth = parseFloat(document.getElementById('deviation-depth').value) || 3000;
        var az = parseFloat(document.getElementById('deviation-azimuth').value) || 0;
        var inc = parseFloat(document.getElementById('deviation-inclination').value) || 60;
        var ns = parseInt(document.getElementById('deviation-stations').value) || 20;
        var resp = await fetch('/api/analysis/deviation-survey', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth:depth,azimuth_deg:az,max_inclination:inc,n_stations:ns})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Stations</strong><br>' + r.n_stations + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>% Stable</strong><br>' + r.pct_stable + '%</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Min SF</strong><br>' + (r.min_safety_factor||0).toFixed(3) + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Stability</strong><br><span class="badge bg-' + (r.stability_class === 'UNSTABLE' || r.stability_class === 'MARGINAL' ? 'danger' : r.stability_class === 'MOSTLY_STABLE' ? 'warning' : 'success') + '">' + (r.stability_class||'-') + '</span></div></div>';
        html += '</div>';
        if (r.critical_station) {
            var cs = r.critical_station;
            html += '<div class="alert alert-warning"><small>Critical station #' + cs.station + ': TVD=' + cs.TVD_m + 'm, Inc=' + cs.inclination_deg + '°, SF=' + cs.sf_collapse + '</small></div>';
        }
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
...
async function runRockStrength() {
    var results = document.getElementById('results-rock-strength');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('well-select') ? document.getElementById('well-select').value : '3P';
        var from = parseFloat(document.getElementById('rockstr-from').value) || 2000;
        var to = parseFloat(document.getElementById('rockstr-to').value) || 4000;
        var lith = document.getElementById('rockstr-lithology').value || 'sandstone';
        var np = parseInt(document.getElementById('rockstr-npoints').value) || 30;
        var resp = await fetch('/api/analysis/rock-strength', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:from,depth_to:to,lithology:lith,n_points:np})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="row mb-3">';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Lithology</strong><br>' + (r.lithology||'-') + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Mean UCS</strong><br>' + (r.mean_UCS_MPa||0) + ' MPa</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>UCS/Sv</strong><br>' + (r.mean_strength_ratio||0).toFixed(3) + '</div></div>';
        html += '<div class="col-md-3"><div class="card p-2 text-center"><strong>Strength</strong><br><span class="badge bg-' + (r.strength_class === 'WEAK' || r.strength_class === 'VERY_WEAK' ? 'danger' : r.strength_class === 'ADEQUATE' ? 'warning' : 'success') + '">' + (r.strength_class||'-') + '</span></div></div>';
        html += '</div>';
        html += '<div class="row mb-2"><div class="col-md-6"><small>UCS Range: ' + r.min_UCS_MPa + ' - ' + r.max_UCS_MPa + ' MPa</small></div>';
        html += '<div class="col-md-6"><small>Depth: ' + r.depth_from_m + ' - ' + r.depth_to_m + 'm (' + r.n_points + ' points)</small></div></div>';
        if (r.plot) html += '<img src="' + r.plot + '" class="img-fluid mb-3">';
        if (r.stakeholder_brief) html += '<div class="alert alert-info"><strong>' + r.stakeholder_brief.headline + '</strong><br>' + r.stakeholder_brief.for_non_experts + '</div>';
        if (r.recommendations) html += '<ul class="list-group">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
...
// ══════════════════════════════════════════════════════
async function runFracPorosityEst() {
    var results = document.getElementById('fracPorosityEstResult');
    results.innerHTML = '<div class="spinner-border text-info"></div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var aperture = parseFloat(document.getElementById('fporosity-aperture').value) || 0.5;
        var binSize = parseFloat(document.getElementById('fporosity-binsize').value) || 50;
        var resp = await fetch('/api/analysis/fracture-porosity-estimate', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({source:'demo', well:well, aperture_mm:aperture, bin_size_m:binSize})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.porosity_class+'</strong> — Mean porosity: '+(r.mean_porosity_pct||0).toFixed(4)+'%, Max: '+(r.max_porosity_pct||0).toFixed(4)+'%</div>';
        ...
        html += '<div class="row"><div class="col-md-4"><div class="card text-center p-2"><h6>Mean Perm</h6><h4>'+(r.mean_permeability_mD||0).toFixed(2)+' mD</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max Perm</h6><h4>'+(r.max_permeability_mD||0).toFixed(2)+' mD</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Bins</h6><h4>'+(r.n_bins||0)+'</h4></div></div></div>';
...
// ══════════════════════════════════════════════════════
async function runDiffStress() {
    var results = document.getElementById('diffStressResult');
    results.innerHTML = '<div class="spinner-border text-info"></div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depFrom = parseFloat(document.getElementById('diffstr-from').value) || 500;
        var depTo = parseFloat(document.getElementById('diffstr-to').value) || 5000;
        var regime = document.getElementById('diffstr-regime').value || 'NF';
        var resp = await fetch('/api/analysis/differential-stress', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({source:'demo', well:well, depth_from:depFrom, depth_to:depTo, n_points:30, regime:regime})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.stress_class+'</strong> — Max differential: '+(r.max_differential_MPa||0).toFixed(1)+' MPa</div>';
        if (r.stakeholder_brief) html += '<div class="alert alert-secondary"><strong>'+r.stakeholder_brief.headline+'</strong><br><small>'+r.stakeholder_brief.for_non_experts+'</small></div>';
        html += '<div class="row"><div class="col-md-4"><div class="card text-center p-2"><h6>Mean Diff</h6><h4>'+(r.mean_differential_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Gradient</h6><h4>'+(r.gradient_MPa_per_km||0).toFixed(2)+' MPa/km</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Points</h6><h4>'+(r.n_points||0)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ══════════════════════════════════════════════════════
async function runFaultSeal() {
    var results = document.getElementById('faultSealResult');
    results.innerHTML = '<div class="spinner-border text-info"></div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var clay = parseFloat(document.getElementById('fseal-clay').value) || 0.2;
        var throwM = parseFloat(document.getElementById('fseal-throw').value) || 50;
        var resp = await fetch('/api/analysis/fault-seal-analysis', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({source:'demo', well:well, clay_fraction:clay, throw_m:throwM})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.seal_class+'</strong> — Mean SGR: '+(r.mean_SGR_pct||0).toFixed(1)+'%, Min SGR: '+(r.min_SGR_pct||0).toFixed(1)+'%</div>';
        if (r.stakeholder_brief) html += '<div class="alert alert-secondary"><strong>'+r.stakeholder_brief.headline+'</strong><br><small>'+r.stakeholder_brief.for_non_experts+'</small></div>';
        html += '<div class="row"><div class="col-md-4"><div class="card text-center p-2"><h6>Max Seal</h6><h4>'+(r.max_seal_capacity_m||0).toFixed(1)+' m</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Zones</h6><h4>'+(r.n_zones||0)+'</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Fractures</h6><h4>'+(r.n_fractures||0)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ══════════════════════════════════════════════════════
// [223] Connectivity Index
// ══════════════════════════════════════════════════════
async function runConnectivityIdx() {
    var results = document.getElementById('connectivityIdxResult');
    results.innerHTML = '<div class="spinner-border text-info"></div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var binSize = parseFloat(document.getElementById('cidx-binsize').value) || 50;
        var dipThresh = parseFloat(document.getElementById('cidx-dipthresh').value) || 30;
        var resp = await fetch('/api/analysis/connectivity-index', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({source:'demo', well:well, bin_size_m:binSize, dip_threshold:dipThresh})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.connectivity_class+'</strong> — Max CI: '+(r.max_connectivity_index||0).toFixed(3)+'</div>';
        if (r.stakeholder_brief) html += '<div class="alert alert-secondary"><strong>'+r.stakeholder_brief.headline+'</strong><br><small>'+r.stakeholder_brief.for_non_experts+'</small></div>';
        html += '<div class="row"><div class="col-md-4"><div class="card text-center p-2"><h6>Mean CI</h6><h4>'+(r.mean_connectivity_index||0).toFixed(3)+'</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Peak Depth</h6><h4>'+(r.max_ci_depth_m||0)+' m</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Above Perc.</h6><h4>'+(r.pct_above_percolation||0).toFixed(0)+'%</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ══════════════════════════════════════════════════════
async function runBreakoutAngular() {
    var results = document.getElementById('breakoutAngularResult');
    results.innerHTML = '<div class="spinner-border text-info"></div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depFrom = parseFloat(document.getElementById('boang-from').value) || 500;
        var depTo = parseFloat(document.getElementById('boang-to').value) || 5000;
        var ucs = parseFloat(document.getElementById('boang-ucs').value) || 80;
        var regime = document.getElementById('boang-regime').value || 'NF';
        var resp = await fetch('/api/analysis/breakout-angular', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({source:'demo', well:well, depth_from:depFrom, depth_to:depTo, n_points:30, UCS_MPa:ucs, regime:regime})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-danger">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-'+(r.risk_class==='CRITICAL'?'danger':r.risk_class==='HIGH'?'warning':'info')+'"><strong>'+r.risk_class+'</strong> — Max breakout: '+(r.max_breakout_width_deg||0).toFixed(1)+'°</div>';
        if (r.stakeholder_brief) html += '<div class="alert alert-secondary"><strong>'+r.stakeholder_brief.headline+'</strong><br><small>'+r.stakeholder_brief.for_non_experts+'</small></div>';
        html += '<div class="row"><div class="col-md-3"><div class="card text-center p-2"><h6>Mean Width</h6><h4>'+(r.mean_breakout_width_deg||0).toFixed(1)+'°</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>First BO</h6><h4>'+(r.first_breakout_depth_m||'N/A')+' m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>% Affected</h6><h4>'+(r.pct_with_breakout||0).toFixed(0)+'%</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>UCS</h6><h4>'+(r.UCS_MPa||0)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// [235] Horizon Stress
// ═══════════════════════════════════════════════════════════════
async function runHorizonStress() {
    var results = document.getElementById('horizonStressResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depthFrom = parseFloat(document.getElementById('hs-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('hs-to').value) || 5000;
        var nPts = parseInt(document.getElementById('hs-npts').value) || 25;
        var regime = document.getElementById('hs-regime').value || 'normal';
        var resp = await fetch('/api/analysis/horizon-stress', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depthFrom,depth_to:depthTo,n_points:nPts,regime:regime})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.horizon_class+'</strong> — Max ratio: '+(r.max_stress_ratio||0).toFixed(3)+', Mean Sv: '+(r.mean_Sv_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max Ratio</h6><h4>'+(r.max_stress_ratio||0).toFixed(3)+'</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Min Eff Shmin</h6><h4>'+(r.min_eff_Shmin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Mean Sv</h6><h4>'+(r.mean_Sv_MPa||0).toFixed(1)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runFractureSwarm() {
    var results = document.getElementById('fractureSwarmResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var windowM = parseFloat(document.getElementById('fsw-window').value) || 10;
        var minCount = parseInt(document.getElementById('fsw-mincount').value) || 3;
        var resp = await fetch('/api/analysis/fracture-swarm-analysis', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,window_m:windowM,min_count:minCount})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.swarm_class+'</strong> — '+r.n_swarms+' swarms, '+(r.pct_in_swarms||0).toFixed(1)+'% clustered</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Swarms</h6><h4>'+r.n_swarms+'</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max Intensity</h6><h4>'+(r.max_intensity_per_m||0).toFixed(3)+'/m</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>In Swarms</h6><h4>'+(r.pct_in_swarms||0).toFixed(1)+'%</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runEffPerm() {
    var results = document.getElementById('effPermResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var aperture = parseFloat(document.getElementById('ep-aperture').value) || 0.1;
        var matrix = parseFloat(document.getElementById('ep-matrix').value) || 0.01;
        var resp = await fetch('/api/analysis/effective-permeability', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,aperture_mm:aperture,matrix_perm_mD:matrix})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.perm_class+'</strong> — k_eff: '+(r.k_eff_mD||0).toFixed(3)+' mD, Enhancement: '+(r.enhancement_factor||0).toFixed(1)+'x</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>k_eff</h6><h4>'+(r.k_eff_mD||0).toFixed(3)+' mD</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>k_connected</h6><h4>'+(r.k_connected_mD||0).toFixed(3)+' mD</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Enhancement</h6><h4>'+(r.enhancement_factor||0).toFixed(1)+'x</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Connectivity</h6><h4>'+(r.connectivity_fraction||0).toFixed(3)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runWellboreTraj() {
    var results = document.getElementById('wellboreTrajResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var dev = parseFloat(document.getElementById('wt-dev').value) || 0;
        var az = parseFloat(document.getElementById('wt-az').value) || 0;
        var depthFrom = parseFloat(document.getElementById('wt-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('wt-to').value) || 5000;
        var resp = await fetch('/api/analysis/wellbore-trajectory', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,deviation_deg:dev,azimuth_well_deg:az,depth_from:depthFrom,depth_to:depthTo})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.traj_class+'</strong> — Optimal azimuth: '+(r.optimal_azimuth_deg||0).toFixed(0)+'°, SHmax est: '+(r.SHmax_est_deg||0).toFixed(1)+'°</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Collapse Margin</h6><h4>'+(r.min_collapse_margin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Frac Margin</h6><h4>'+(r.min_frac_margin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Optimal Az</h6><h4>'+(r.optimal_azimuth_deg||0).toFixed(0)+'°</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Worst Az</h6><h4>'+(r.worst_azimuth_deg||0).toFixed(0)+'°</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runStressAniso() {
    var results = document.getElementById('stressAnisoResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depthFrom = parseFloat(document.getElementById('sar-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('sar-to').value) || 5000;
        var nPts = parseInt(document.getElementById('sar-npts').value) || 25;
        var resp = await fetch('/api/analysis/stress-anisotropy-ratio', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depthFrom,depth_to:depthTo,n_points:nPts})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.aniso_class+'</strong> — Max anisotropy: '+(r.max_aniso_ratio||0).toFixed(4)+', Mean diff stress: '+(r.mean_diff_stress_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Mean Aniso</h6><h4>'+(r.mean_aniso_ratio||0).toFixed(4)+'</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Max Aniso</h6><h4>'+(r.max_aniso_ratio||0).toFixed(4)+'</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Mean Diff</h6><h4>'+(r.mean_diff_stress_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Max Deviatoric</h6><h4>'+(r.max_deviatoric_ratio||0).toFixed(4)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runPpWindow() {
    var results = document.getElementById('ppWindowResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depthFrom = parseFloat(document.getElementById('ppw-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('ppw-to').value) || 5000;
        var nPts = parseInt(document.getElementById('ppw-npts').value) || 25;
        var mw = parseFloat(document.getElementById('ppw-mw').value) || 10.0;
        var resp = await fetch('/api/analysis/pore-pressure-window', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depthFrom,depth_to:depthTo,n_points:nPts,mud_weight_ppg:mw})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.pp_class+'</strong> — Min window: '+(r.min_window_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Min Window</h6><h4>'+(r.min_window_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Min Kick Margin</h6><h4>'+(r.min_kick_margin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Min Loss Margin</h6><h4>'+(r.min_loss_margin_MPa||0).toFixed(1)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runFracDensityProf() {
    var results = document.getElementById('fracDensityProfResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var binSize = parseFloat(document.getElementById('fdp-bin').value) || 50;
        var resp = await fetch('/api/analysis/fracture-density-profile', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,bin_size_m:binSize})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.density_class+'</strong> — Max: '+(r.max_density_per_m||0).toFixed(4)+'/m, Mean: '+(r.mean_density_per_m||0).toFixed(4)+'/m</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Bins</h6><h4>'+r.n_bins+'</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Max Density</h6><h4>'+(r.max_density_per_m||0).toFixed(4)+'/m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Mean Density</h6><h4>'+(r.mean_density_per_m||0).toFixed(4)+'/m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>CV</h6><h4>'+(r.cv_density||0).toFixed(3)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runStressPolygon() {
    var results = document.getElementById('stressPolygonResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth = parseFloat(document.getElementById('sp-depth').value) || 3000;
        var friction = parseFloat(document.getElementById('sp-friction').value) || 0.6;
        var resp = await fetch('/api/analysis/stress-polygon-frictional', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth:depth,friction:friction})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.current_regime+'</strong> — Sv='+(r.Sv_MPa||0).toFixed(1)+', Shmin='+(r.Shmin_est_MPa||0).toFixed(1)+', SHmax='+(r.SHmax_est_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Sv</h6><h4>'+(r.Sv_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Shmin</h6><h4>'+(r.Shmin_est_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>SHmax</h6><h4>'+(r.SHmax_est_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>q</h6><h4>'+(r.frictional_limit_q||0).toFixed(3)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runRockStrength() {
    ...
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depthFrom = parseFloat(document.getElementById('rs-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('rs-to').value) || 5000;
        var nPts = parseInt(document.getElementById('rs-npts').value) || 25;
        var lith = document.getElementById('rs-lith').value || 'sandstone';
        var resp = await fetch('/api/analysis/rock-strength-lithology', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depthFrom,depth_to:depthTo,n_points:nPts,lithology:lith})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.strength_class+'</strong> ('+r.lithology+') — UCS range: '+(r.min_UCS_MPa||0).toFixed(0)+'-'+(r.max_UCS_MPa||0).toFixed(0)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Mean UCS</h6><h4>'+(r.mean_UCS_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Min UCS</h6><h4>'+(r.min_UCS_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max UCS</h6><h4>'+(r.max_UCS_MPa||0).toFixed(1)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ═══════════════════════════════════════════════════════════════
async function runBreakoutDepth() {
    var results = document.getElementById('breakoutDepthResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depthFrom = parseFloat(document.getElementById('bd-from').value) || 500;
        var depthTo = parseFloat(document.getElementById('bd-to').value) || 5000;
        var nPts = parseInt(document.getElementById('bd-npts').value) || 25;
        var ucs = parseFloat(document.getElementById('bd-ucs').value) || 50;
        var resp = await fetch('/api/analysis/wellbore-breakout-depth', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depthFrom,depth_to:depthTo,n_points:nPts,UCS_MPa:ucs})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.breakout_class+'</strong> — '+(r.pct_breakout||0).toFixed(1)+'% affected, min margin: '+(r.min_margin_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Breakout Zones</h6><h4>'+r.n_breakout_zones+'</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>% Affected</h6><h4>'+(r.pct_breakout||0).toFixed(1)+'%</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min Margin</h6><h4>'+(r.min_margin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min SF</h6><h4>'+(r.min_stability_factor||0).toFixed(3)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// [250] ECD Profile
// ============================================================
async function runEcdProfile() {
    var results = document.getElementById('ecdProfileResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('ecd-from').value) || 500;
        var depth_to = parseFloat(document.getElementById('ecd-to').value) || 5000;
        var mud_weight_ppg = parseFloat(document.getElementById('ecd-mw').value) || 10;
        var flow_rate_gpm = parseFloat(document.getElementById('ecd-flow').value) || 500;
        var resp = await fetch('/api/analysis/ecd-profile', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,mud_weight_ppg:mud_weight_ppg,flow_rate_gpm:flow_rate_gpm})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.ecd_class+'</strong> — Max ECD: '+(r.max_ecd_ppg||0).toFixed(2)+' ppg</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Max ECD</h6><h4>'+(r.max_ecd_ppg||0).toFixed(2)+' ppg</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min Kick Margin</h6><h4>'+(r.min_kick_margin_ppg||0).toFixed(2)+' ppg</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min Loss Margin</h6><h4>'+(r.min_loss_margin_ppg||0).toFixed(2)+' ppg</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>MW</h6><h4>'+(r.mud_weight_ppg||0).toFixed(1)+' ppg</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ============================================================
async function runFracSpacing() {
    var results = document.getElementById('fracSpacingResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var resp = await fetch('/api/analysis/fracture-spacing-analysis', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.spacing_class+'</strong> — Mean spacing: '+(r.mean_spacing_m||0).toFixed(2)+' m, CV: '+(r.cv||0).toFixed(3)+'</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Mean</h6><h4>'+(r.mean_spacing_m||0).toFixed(2)+' m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Median</h6><h4>'+(r.median_spacing_m||0).toFixed(2)+' m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min</h6><h4>'+(r.min_spacing_m||0).toFixed(2)+' m</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Max</h6><h4>'+(r.max_spacing_m||0).toFixed(2)+' m</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ============================================================
async function runOverburdenGrad() {
    var results = document.getElementById('overburdenGradResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('ob-from').value) || 100;
        var depth_to = parseFloat(document.getElementById('ob-to').value) || 5000;
        var n_points = parseInt(document.getElementById('ob-npts').value) || 25;
        var resp = await fetch('/api/analysis/overburden-gradient', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,n_points:n_points})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.gradient_class+'</strong> — Mean gradient: '+(r.mean_gradient_psi_ft||0).toFixed(4)+' psi/ft</div>';
        html += '<div class="row g-2 mb-2">';
        ...
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Mean EMW</h6><h4>'+(r.mean_emw_ppg||0).toFixed(2)+' ppg</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max Sv</h6><h4>'+(r.max_Sv_MPa||0).toFixed(1)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ============================================================
async function runStressRotation() {
    var results = document.getElementById('stressRotationResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var n_zones = parseInt(document.getElementById('sr-nzones').value) || 3;
        var depth_from = parseFloat(document.getElementById('sr-from').value) || 500;
        var resp = await fetch('/api/analysis/principal-stress-rotation', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,n_zones:n_zones,depth_from:depth_from})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.rotation_class+'</strong> — Max rotation: '+(r.max_rotation_deg||0).toFixed(1)+'°</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Max Rotation</h6><h4>'+(r.max_rotation_deg||0).toFixed(1)+'°</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Mean SHmax</h6><h4>'+(r.mean_SHmax_deg||0).toFixed(1)+'°</h4></div></div>';
        html += '<div class="col-md-4"><div class="card text-center p-2"><h6>Zones</h6><h4>'+(r.n_zones||0)+'</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
// ============================================================
async function runTensileFailure() {
    var results = document.getElementById('tensileFailureResult');
    results.innerHTML = '<div class="text-center"><div class="spinner-border spinner-border-sm"></div> Running...</div>';
    showLoading();
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('tf-from').value) || 500;
        var depth_to = parseFloat(document.getElementById('tf-to').value) || 5000;
        var tensile_MPa = parseFloat(document.getElementById('tf-tensile').value) || 10;
        var n_points = parseInt(document.getElementById('tf-npts').value) || 25;
        var resp = await fetch('/api/analysis/wellbore-tensile-failure', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,tensile_MPa:tensile_MPa,n_points:n_points})});
        var r = await resp.json();
        if (r.error) { results.innerHTML = '<div class="alert alert-warning">'+r.error+'</div>'; return; }
        var html = '<div class="alert alert-info"><strong>'+r.tensile_class+'</strong> — Min margin: '+(r.min_margin_MPa||0).toFixed(1)+' MPa</div>';
        html += '<div class="row g-2 mb-2">';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min Margin</h6><h4>'+(r.min_margin_MPa||0).toFixed(1)+' MPa</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Frac Zones</h6><h4>'+(r.n_frac_zones||0)+'</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>% Fractured</h6><h4>'+(r.pct_fractured||0).toFixed(1)+'%</h4></div></div>';
        html += '<div class="col-md-3"><div class="card text-center p-2"><h6>Min FIP</h6><h4>'+(r.min_fip_MPa||0).toFixed(1)+' MPa</h4></div></div></div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runDirectionalStability() {
    var results = document.getElementById('directionalStabilityResult');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('ds-from').value) || 500;
        var depth_to = parseFloat(document.getElementById('ds-to').value) || 5000;
        var azi = parseFloat(document.getElementById('ds-azi').value) || 0;
        var inc = parseFloat(document.getElementById('ds-inc').value) || 0;
        var ucs = parseFloat(document.getElementById('ds-ucs').value) || 50;
        var resp = await fetch('/api/analysis/directional-stability', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,n_points:30,wellbore_azimuth_deg:azi,wellbore_inclination_deg:inc,UCS_MPa:ucs})});
        var r = await resp.json();
        var html = '<div class="alert alert-'+(r.dir_class==='CRITICAL'||r.dir_class==='UNSTABLE'?'danger':r.dir_class==='MARGINAL'?'warning':'success')+'"><strong>'+r.dir_class+'</strong> — Min SI '+r.min_stability_index+', '+r.pct_unstable+'% unstable</div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runEffStressGrad() {
    var results = document.getElementById('effStressGradResult');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('esg-from').value) || 500;
        var depth_to = parseFloat(document.getElementById('esg-to').value) || 5000;
        var ppg = parseFloat(document.getElementById('esg-ppg').value) || 0.45;
        var resp = await fetch('/api/analysis/effective-stress-gradient', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,n_points:30,Pp_gradient:ppg})});
        var r = await resp.json();
        var html = '<div class="alert alert-'+(r.grad_class==='LOW_CONFINING'?'danger':r.grad_class==='MODERATE'?'warning':'success')+'"><strong>'+r.grad_class+'</strong> — Shmin/Sv ratio '+r.mean_stress_ratio+'</div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runDepletionEffect() {
    var results = document.getElementById('depletionEffectResult');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth = parseFloat(document.getElementById('dep-depth').value) || 3000;
        var dp = parseFloat(document.getElementById('dep-dp').value) || 10;
        var nu = parseFloat(document.getElementById('dep-nu').value) || 0.25;
        var resp = await fetch('/api/analysis/depletion-effect', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_m:depth,depletion_MPa:dp,poisson_ratio:nu})});
        var r = await resp.json();
        var html = '<div class="alert alert-'+(r.depl_class==='SEVERE'?'danger':r.depl_class==='SIGNIFICANT'?'warning':'success')+'"><strong>'+r.depl_class+'</strong> — Frac gradient change '+r.total_frac_change_MPa+' MPa</div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runFracReopen() {
    var results = document.getElementById('fracReopenResult');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth_from = parseFloat(document.getElementById('frp-from').value) || 500;
        var depth_to = parseFloat(document.getElementById('frp-to').value) || 5000;
        var resp = await fetch('/api/analysis/fracture-reopen-pressure', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_from:depth_from,depth_to:depth_to,n_points:30})});
        var r = await resp.json();
        var html = '<div class="alert alert-'+(r.reopen_class==='LOW_MARGIN'?'danger':r.reopen_class==='MODERATE'?'warning':'success')+'"><strong>'+r.reopen_class+'</strong> — Mean margin '+r.mean_margin_MPa+' MPa</div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
...
async function runAPB() {
    var results = document.getElementById('apbResult');
    showLoading(); results.innerHTML = '';
    try {
        var well = document.getElementById('wellSelect') ? document.getElementById('wellSelect').value : '3P';
        var depth = parseFloat(document.getElementById('apb-depth').value) || 3000;
        var pt = parseFloat(document.getElementById('apb-pt').value) || 120;
        var it = parseFloat(document.getElementById('apb-it').value) || 40;
        var fluid = document.getElementById('apb-fluid').value || 'water';
        var od = parseFloat(document.getElementById('apb-od').value) || 9.625;
        var resp = await fetch('/api/analysis/annular-pressure-buildup', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({source:'demo',well:well,depth_m:depth,production_temp_C:pt,initial_temp_C:it,annulus_fluid:fluid,casing_od_in:od})});
        var r = await resp.json();
        var html = '<div class="alert alert-'+(r.apb_class==='CRITICAL'||r.apb_class==='HIGH_RISK'?'danger':r.apb_class==='MODERATE'?'warning':'success')+'"><strong>'+r.apb_class+'</strong> — Max APB '+r.max_apb_MPa+' MPa ('+r.max_apb_psi+' psi), SF='+r.min_safety_factor+'</div>';
        if (r.plot) html += '<img src="'+r.plot+'" class="img-fluid mt-2" />';
        if (r.recommendations) html += '<ul class="list-group mt-2">' + r.recommendations.map(function(x){return '<li class="list-group-item">'+x+'</li>';}).join('') + '</ul>';
        html += '<small class="text-muted">Elapsed: ' + (r.elapsed_s || 0).toFixed(3) + 's</small>';
        results.innerHTML = html;
