...
print("\n[72] Conformal Prediction")
cf = api("POST", "/api/analysis/conformal-predict", {"well": "3P", "source": "demo"}, timeout=60)
...
check("Has coverage_target", isinstance(cf.get("coverage_target"), (int, float)))
check("Has actual_coverage", isinstance(cf.get("actual_coverage"), (int, float)) and cf["actual_coverage"] > 0)
check("Has avg_set_size", isinstance(cf.get("avg_set_size"), (int, float)) and cf["avg_set_size"] >= 1)
check("Has singleton_pct", isinstance(cf.get("singleton_pct"), (int, float)))
check("Has class_analysis", isinstance(cf.get("class_analysis"), list) and len(cf["class_analysis"]) >= 2)
ca0 = cf["class_analysis"][0]
check("CA has class", isinstance(ca0.get("class"), str))
check("CA has count", isinstance(ca0.get("count"), int))
check("CA has avg_set_size", isinstance(ca0.get("avg_set_size"), (int, float)))
check("CA has singleton_pct", isinstance(ca0.get("singleton_pct"), (int, float)))
check("CA has coverage", isinstance(ca0.get("coverage"), (int, float)))
check("CA has confidence", ca0.get("confidence") in ("HIGH", "MEDIUM", "LOW"))
check("Has uncertain_samples", isinstance(cf.get("uncertain_samples"), list))
